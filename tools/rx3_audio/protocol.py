...
class StreamConfig:
    ...
    def validate(self) -> None:
        ...
        if self.sample_format != S16_LE:
            raise ProtocolError(f"unsupported sample format {self.sample_format}")
        if not 1 <= self.frames_per_block <= self.sample_rate:
            raise ProtocolError(f"invalid frames per block {self.frames_per_block}")
...
def encode_message(
    ...
    if message_type not in (CONFIG, PCM, HEARTBEAT):
