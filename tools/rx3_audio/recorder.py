...
from .protocol import CONFIG, PCM, Decoder, Message, ProtocolError, StreamConfig


LOG = logging.getLogger("rx3-audio-recorder")
WAV_HEADER = struct.Struct("<4sI4s4sIHHIIHH4sI")
UINT32_MAX = (1 << 32) - 1
MAX_WAV_DATA_BYTES = UINT32_MAX - 36
...
class WavWriter:
    ...
    def _write_header(self) -> None:
        if self.data_bytes > MAX_WAV_DATA_BYTES:
    ...
    def write(self, pcm: bytes) -> None:
        ...
        if len(pcm) > MAX_WAV_DATA_BYTES - self.data_bytes:
            raise OverflowError(
                "recording reached the 4 GiB RIFF/WAV limit; start a new output file"
            )
...
class StreamRecorder:
    ...
    def handle(self, message: Message) -> None:
        if message.type == CONFIG:
            ...
            if self._expected_timestamp is not None:
                if message.timestamp_frames > self._expected_timestamp:
                    self.stats.missing_frames += (
                        message.timestamp_frames - self._expected_timestamp
                    )
        ...
        if frame_count > self.config.max_frames_per_block:
            raise ProtocolError(
                f"PCM block has {frame_count} frames; configured maximum is "
                f"{self.config.max_frames_per_block}"
            )
