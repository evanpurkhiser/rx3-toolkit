...
The `pcm-stream` module hooks the firmware 1.19 master-recorder bus and serves
signed 16-bit little-endian stereo PCM at 44,100 Hz on TCP port 7355.
...
The runtime packager supplies the firmware key and builds the module as an
independent option. The stream works through any configured RX3 IPv4 interface
and does not enable a root shell.
...
declares 44,100 Hz, two channels, PCM16LE, and the maximum accepted block size.
...
The callback copies a complete block into a preallocated SPSC ring and publishes
its write index with release semantics. It performs no allocation, locking,
file operation, socket operation, conversion, or wait. A full ring or invalid
block is dropped whole and counted. A worker owns Float32-to-PCM16 conversion
and every TCP operation. One absolute send deadline disconnects a stalled
client rather than allowing receiver backpressure into the audio thread.
...
without errors. This established the hook, conversion, framing, USB Ethernet
path, and host WAV writer on hardware. That run used the earlier 32-slot queue.
The current split sample/descriptor queue, nonblocking send deadline, runtime
configuration restart, and status reporting still need a hardware smoke test.
Channel identity and the recorder microphone setting also need controlled source
tests.
