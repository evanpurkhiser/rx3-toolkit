...
podman run --rm --network=none \
    ...
    localhost/rx3-reverse:latest sh -eu -c '
        cd /src
        python3 mod/modules/pcm-stream/1.19/test_regressions.py
        clang --target=arm-linux-gnueabi \
            ...
            -o /out/librx3_pcm_stream.so \
            mod/modules/pcm-stream/1.19/rx3_pcm_stream.c
        file /out/librx3_pcm_stream.so
        readelf -h /out/librx3_pcm_stream.so
        sha256sum /out/librx3_pcm_stream.so
    '
