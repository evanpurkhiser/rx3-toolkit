...
podman run --rm --network=none \
    ...
    localhost/rx3-reverse:latest sh -eu -c '
        clang --target=arm-linux-gnueabi \
            ...
            -o /out/librx3_link_bootstrap.so \
            /src/tools/rx3_link_export/rx3_link_bootstrap.c
        file /out/librx3_link_bootstrap.so
        readelf -h /out/librx3_link_bootstrap.so
        sha256sum /out/librx3_link_bootstrap.so
    '
