...
from tools.rx3_audio.recorder import StreamRecorder, WavWriter
...
class StreamRecorderTest(unittest.TestCase):
    def test_tracks_gaps_and_reconnects(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            recorder.handle(Message(HEARTBEAT, 0, 12, 102, 5, b""))
            recorder.handle(Message(PCM, 0, 14, 105, 8, b"\x00" * 8))
            recorder.begin_connection()
