...
class Rx3SshTests(unittest.TestCase):
    def test_module_is_opt_in_and_depends_on_bridge(self) -> None:
        self.assertFalse(MANIFEST["default"])
        self.assertIn("s3-link-bridge", MANIFEST["requires"])
        self.assertNotIn("usb-link-root-shell", MANIFEST["conflicts"])
        self.assertNotIn("telnet", MANIFEST["conflicts"])
        self.assertIn("usb-serial", MANIFEST["conflicts"])

    def test_packaged_binary_matches_reproducible_build(self) -> None:
        expected = hashlib.sha256(
            (BUILD_TOOL / "build/dropbearmulti").read_bytes()
        ).digest()
        actual = hashlib.sha256((HERE / "dropbearmulti").read_bytes()).digest()
    ...
    def test_private_material_is_generated_at_runtime(self) -> None:
        ...
        self.assertEqual(packaged, {"module.sh"})
        self.assertIn("RX3_SSH_CONFIG=$USB/RX3_SSH", MODULE)
        self.assertIn("RX3_SSH_SOURCE=$RX3_SSH_CONFIG/dropbearmulti", MODULE)
