...
{
    int fd = open("/tmp/rx3-link-export-activate.log",
...
static void *activate_link_export(void *unused)
{
    ...
    if (!firmware_guards_match()) {
        log_line("link export activate: firmware guard rejected\n");
        return 0;
    }
    ...
    if (attempt == 30) {
        log_line("link export activate: PcController graph unavailable\n");
        return 0;
    }
    ...
    if (*((volatile unsigned char *)pc_controller + 0x72u) != 1) {
        log_line("link export activate: stock callback did not set mounted flag\n");
        return 0;
    }
    ...
    if (change_link_stop_state(SYSTEM_STATE_CONNECTING))
        log_line("link export activate: LinkStop resumed at Connecting\n");
    return 0;
}
...
__attribute__((constructor)) static void initialize(void)
{
    ...
    if (!running_in_rbp())
        return;
    if (pthread_create(&worker, 0, activate_link_export, 0)) {
        log_line("link export activate: worker creation failed\n");
        return;
    }
